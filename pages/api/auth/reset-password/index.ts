...
export const resetPassword = async (
  ...
  const resetData = await prisma.passwordResets.findUnique({
  ...
  const updatedPasswordRecord = await prisma.passwordResets.update({
    data: { valid: false },
  ...
  const userRecord = await prisma.user.update({
    ...
    where: { id: updatedPasswordRecord.userId },
  });
